...
class Controller_GeneralController(ctrl.MetaController):
    ...
    @ctrl.controllefunction(level=ctrl.LEVEL["VIEW"])
    def get_html(self, level=ctrl.LEVEL["NONE"]):
        data = super().get_html(self,level=level)
        ...
        for position in range(len(self.required_controller)):
            ...
            if boardcontroller is not None:
                ...
                for title, input in boardcontroller.get_controller_webfunctions(
                    ...
                    input_subs.append(
                        ctrl.gen_html_json(
                            ...
                            )]+input.html_controller()+[ctrl.gen_html_json(
                                node_type="button",
                                text="set",
                                attributes={'class':['btn', 'btn-primary', 'board_var_set_button'],'controller_id':'{{controller_id}}','position':position},
                            )]
                            ,
                            attributes={'class':['form-group', 'controller_input_group']},
                        )
                ...
                container_subs.append(ctrl.gen_html_json(
                    node_type="div",
    ...
    @ctrl.controllefunction(level=ctrl.LEVEL["EDIT"])
